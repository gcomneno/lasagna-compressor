...
def huffman_compress_core(data: bytes) -> Tuple[List[int], int, bytes]:
    """
    Core riusabile (Step1/Step2/Step3): data -> (freq, lastbits, bitstream)
    """
...
# -------------------
# Qui lavoriamo direttamente sui byte, ma consideriamo "lettere" solo ASCII [A-Za-z]
...
def compress_bytes_v3(data: bytes) -> bytes:
    ...
    Dove:
    - costruisco token (sillabe + blocchi non-lettera)
    - creo un vocab (token -> id 0..K-1)
    - creo una sequenza di id come byte (id <=255)
    - applico Huffman sugli id.
    """
...
def decompress_bytes_v3(comp: bytes) -> bytes:
    ...
    if len(id_bytes) != N_tokens:
        # Potremmo essere più tolleranti, ma in teoria devono coincidere
        pass
...
def main(argv: List[str]) -> int:
    if len(argv) < 4 or argv[1] not in ("c1", "d1", "c2", "d2", "c3", "d3"):
        print(f"Uso:")
